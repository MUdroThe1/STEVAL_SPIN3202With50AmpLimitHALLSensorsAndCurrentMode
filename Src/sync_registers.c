  ...
  switch(typeID)
  {
    ...
    case TYPE_DATA_16BIT:
    {
      ...
      switch (regID)
      {
        ...
        case MC_REG_CURR_MONITOR_SAMPL_NS:
        {
          RCM_SetCurrentMonitorDist2Edge(&CurrMonitor_M1, regdata16);
          break;
        }

        case MC_REG_CURR_MONITOR_AVG:
        {
          RCM_SetCurrentMonitorAvg(&CurrMonitor_M1, (uint8_t) regdata16);
          break;
        }
      ...
      case TYPE_DATA_16BIT:
      {
        ...
        if (freeSpace >= 2)
        {
          switch (regID)
          {
            ...
            case MC_REG_CURR_MONITOR:
            {
              *regdata16 = RCM_GetCurrentMonitor(&CurrMonitor_M1);
              break;
            }
            ...
            case MC_REG_CURR_MONITOR_SAMPL_NS:
            {
              *regdataU16 = RCM_GetCurrentMonitorDist2Edge(&CurrMonitor_M1);
              break;
            }

            case MC_REG_CURR_MONITOR_AVG:
            {
              *regdataU16 = RCM_GetCurrentMonitorAvg(&CurrMonitor_M1);
              break;
            }
